...
# MODEL = "llama-3.3-70b-versatile"  # stronger model — fewer subtle bugs per attempt
...
# def run_aider(task: str) -> dict:
#     """Send one task to Aider, non-interactively, scoped to workspace/.
#     Aider is run WITH its working directory set to workspace/ itself, so
#     any file it creates on its own initiative also lands inside workspace/
#     instead of leaking to the project root."""
#     py_files = set(glob.glob(os.path.join(WORKSPACE_DIR, "*.py")))

#     for match in re.findall(r"\b([\w\-/]+\.py)\b", task):
#         filename = os.path.basename(match)
#         py_files.add(os.path.join(WORKSPACE_DIR, filename))

#     if not py_files:
#         py_files = {os.path.join(WORKSPACE_DIR, "detector.py")}

#     # Convert to basenames since Aider will run with cwd=workspace/
#     file_basenames = sorted(os.path.basename(p) for p in py_files)

#     cmd = [
#         "aider",
#         "--model", f"groq/{MODEL}",
#         "--api-key", f"groq={GROQ_API_KEY}",
#         "--edit-format", "whole",
#         "--yes-always",
#         "--no-stream",
#         "--map-tokens", "0",
...
#         )
#         return {
#             "stdout": result.stdout[-4000:],  # keep it bounded
...
#     """Diff of the last change Aider made inside workspace/."""
#     result = subprocess.run(
...
# def build_feedback_message(task: str, aider_result: dict, diff: str, test_result: dict) -> str:
#     """Everything the Planner needs to actually 'see' the real outcome."""
#     return f"""TASK ATTEMPTED:
# {task}
...
#         {"role": "user", "content": (
#             "The previous run reported DONE and all pytest tests passed, but "
#             "those tests used mocked API responses. I then ran detect() "
#             "against a REAL document (real Groq API call, no mock) and got "
#             "an empty result every time: {\"duplicates\": [], \"contradictions\": []}. "
#             "Investigating detector.py, the bug is: the code reads "
#             "result[\"choices\"][0][\"text\"] from the Groq response, but "
#             "Groq's chat completions API actually returns the response at "
#             "result[\"choices\"][0][\"message\"][\"content\"]. Since [\"text\"] "
#             "doesn't exist, a KeyError is raised and silently caught, always "
#             "returning an empty result. There is also a second, separate bug: "
#             "parse_bullets uses line[1] == '.' to detect the end of the "
#             "bullet number, which only works for single-digit IDs (1-9) and "
#             "silently skips any bullet numbered 10 or higher. "
#             "Give me the first task to fix these two real, confirmed bugs in "
#             "the existing detector.py."
#         )},
#     ]
...
#         log_event({"iteration": i, "stage": "planner_decision", **plan})

#         if not plan["decision"] or not plan["task"]:
#             print("\n--- RAW PLANNER RESPONSE (parsing failed) ---")
#             print(plan["raw"])
#             print("--- END RAW RESPONSE ---\n")

#         if plan["decision"].upper() == "DONE":
#             print("\nPlanner reports DONE. Stopping loop.")
#             log_event({"iteration": i, "stage": "run_complete"})
#             break

#         if not plan["task"]:
#             print("Planner returned no task. Stopping to avoid a blind loop.")
#             log_event({"iteration": i, "stage": "aborted_no_task"})
#             break

#         aider_result = run_aider(plan["task"])
#         diff = get_git_diff()
#         test_result = run_tests()

#         log_event({
#             "iteration": i, "stage": "aider_result",
#             "aider_result": aider_result, "diff": diff, "test_result": test_result,
#         })
...
#             print("\nSame failure repeated 3 times. Stopping loop (safety control).")
#             log_event({"iteration": i, "stage": "aborted_repeated_failure"})
#             break

#         feedback = build_feedback_message(plan["task"], aider_result, diff, test_result)
...
#     """Send the task directly to Groq and write the returned file content
#     straight to disk ourselves — no third-party CLI tool in between, so
#     there's no file-resolution logic left to misfire."""
#     current_detector = _read_file(DETECTOR_PATH) or "(empty — does not exist yet)"
#     current_test = _read_file(TEST_DETECTOR_PATH) or "(empty — does not exist yet)"

#     messages = [
#         {"role": "system", "content": CODING_AGENT_PROMPT},
#         {"role": "user", "content": (
#             f"TASK: {task}\n\n"
#             f"CURRENT detector.py:\n{current_detector}\n\n"
#             f"CURRENT test_detector.py:\n{current_test}\n\n"
#             "Respond with the complete new content of both files using the "
#             "exact marker format."
#         )},
#     ]
...
#             last_error = e
#             wait = 5 * (attempt + 1)
#             print(f"Coding Agent call failed ({e}). Retrying in {wait}s...")
#             time.sleep(wait)
#     else:
#         return {
#             "stdout": f"Coding Agent call failed after 3 retries: {last_error}",
...
#         r"<<<DETECTOR_PY_START>>>(.*?)<<<DETECTOR_PY_END>>>", text, re.DOTALL
#     )
#     test_match = re.search(
#         r"<<<TEST_DETECTOR_PY_START>>>(.*?)<<<TEST_DETECTOR_PY_END>>>", text, re.DOTALL
#     )

#     wrote = []
#     if detector_match:
#         content = detector_match.group(1).strip() + "\n"
#         os.makedirs(WORKSPACE_DIR, exist_ok=True)
#         with open(DETECTOR_PATH, "w", encoding="utf-8") as f:
#             f.write(content)
#         wrote.append("detector.py")
#     if test_match:
#         content = test_match.group(1).strip() + "\n"
#         os.makedirs(WORKSPACE_DIR, exist_ok=True)
#         with open(TEST_DETECTOR_PATH, "w", encoding="utf-8") as f:
#             f.write(content)
#         wrote.append("test_detector.py")

#     # Commit directly ourselves, from the project root
#     if wrote:
#         try:
#             subprocess.run(["git", "add", DETECTOR_PATH, TEST_DETECTOR_PATH],
#                             capture_output=True, text=True)
#             subprocess.run(["git", "commit", "-m", f"task: {task[:60]}"],
#                             capture_output=True, text=True)
#         except Exception:
#             pass

#     summary = f"Wrote: {', '.join(wrote) if wrote else 'NOTHING — markers not found in response'}"
#     return {
#         "stdout": summary + "\n\n--- RAW MODEL RESPONSE ---\n" + text[:2000],
#         "stderr": "",
#         "returncode": 0 if wrote else 1,
#         "timed_out": False,
#     }
...
#     mock. This closes the exact gap that let the loop declare DONE on code
#     that had a wrong model name and wrong request format: mocked unit tests
#     can pass on completely broken real integration, since mocks don't
#     exercise the real network call at all. This function does."""
...
#         "    text = ('1. The rate for Material A shall be Rs. 50 per kg.\\n'\n"
...
#         "        sys.exit(1)\n"
#         "    has_duplicate = len(result['duplicates']) > 0\n"
#         "    has_contradiction = len(result['contradictions']) > 0\n"
#         "    if not has_duplicate or not has_contradiction:\n"
#         "        print(f'FAIL: expected at least one duplicate AND one contradiction on this known real example, got: {result}')\n"
#         "        sys.exit(1)\n"
...
# def build_feedback_message(task: str, aider_result: dict, diff: str, test_result: dict, real_check: dict) -> str:
#     """Everything the Planner needs to actually 'see' the real outcome."""
#     return f"""TASK ATTEMPTED:
# {task}
...
#         feedback = build_feedback_message(precise_task, aider_result, diff, test_result, real_check)
...
def run_build_loop():
    """Generator version of the build loop — yields a progress dict after
    every real step, so a caller (CLI or Streamlit) can render live
    updates as they happen, without needing a separate background process
    ...
    for i in range(1, MAX_ITERATIONS + 1):
        ...
        log_event({"iteration": i, "stage": "supervisor_decision", **plan})
        yield {"iteration": i, "stage": "supervisor_decision", **plan}
        ...
        if decision.upper() == "DONE":
            if last_test_result.get("passed") and last_real_check.get("passed"):
                ...
                save_project_state(project_state)
                log_event({"iteration": i, "stage": "run_complete"})
                yield {"iteration": i, "stage": "run_complete"}
                return
            else:
                log_event({"iteration": i, "stage": "done_overridden_real_check_failed"})
                yield {"iteration": i, "stage": "done_overridden_real_check_failed",
        ...
        if not focus:
            log_event({"iteration": i, "stage": "aborted_no_task"})
            yield {"iteration": i, "stage": "aborted_no_task"}
            return

        context = get_all_workspace_files() or "(workspace is empty)"
        translated = call_task_agent(focus, reason, context)
        log_event({"iteration": i, "stage": "task_agent_translation", **translated})
        yield {"iteration": i, "stage": "task_agent_translation", **translated}

        precise_task = translated["task"] or focus
        ...
        if recent_failure_signatures.count(failure_signature) >= MAX_REPEATED_FAILURES:
            log_event({"iteration": i, "stage": "aborted_repeated_failure"})
            yield {"iteration": i, "stage": "aborted_repeated_failure"}
            return
    ...
    log_event({"stage": "aborted_max_iterations"})
    yield {"stage": "aborted_max_iterations"}


def main():
    """CLI entry point — just prints each event as the generator produces it."""
    for event in run_build_loop():
        stage = event.get("stage")
        it = event.get("iteration")

        if stage == "supervisor_decision":
            print(f"\n=== Iteration {it} ===")
            print(f"DECISION: {event.get('decision')}")
            print(f"FOCUS: {event.get('task')}")
            print(f"REASON: {event.get('reason')}")
        elif stage == "task_agent_translation":
            print(f"TASK AGENT TASK: {event.get('task')}")
        elif stage == "aider_result":
            print(f"CODING AGENT TRANSCRIPT:\n{event['aider_result']['stdout']}\n")
        elif stage == "run_complete":
            print("\nSupervisor reports DONE, and both mocked tests AND "
                  "the real API integration check have genuinely passed. "
                  "Stopping loop.")
        elif stage == "done_overridden_real_check_failed":
            print("\nSupervisor said DONE, but the real integration check "
                  "has not actually passed yet — overriding and forcing "
                  "another FIX round with the real evidence.")
        elif stage == "aborted_no_task":
            print("Supervisor returned no focus. Stopping to avoid a blind loop.")
        elif stage == "aborted_repeated_failure":
            print("\nSame failure repeated. Stopping loop (safety control).")
        elif stage == "aborted_max_iterations":
            print(f"\nReached max iterations ({MAX_ITERATIONS}). Stopping.")
